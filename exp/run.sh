...
echo "q     Queue priority mode [Default 0:traffic-aware, 1:time-sorted, 2:round-robin]"
...
run_batch(){
  ...
  if [ $write_thru_post -eq 99 ]; then # Not set in the options, default value for apps
    if [ $a -eq 1 ] || [ $a -eq 4 ] || [ $a -eq 5 ] || [ $a -eq 7 ] || [ $a -eq 8 ]; then #Apps without frontier or with barrier
      write_thru_post=0
    else # 0 2 3
      write_thru_post=1
    fi
  ...
  let core_threads=$max_threads/2
  # Determine number of threads to use
  if [[ $grid_x -gt $core_threads ]]
  then
  ...
  compiler_options=""
  target=""
  target=1 # 0:any, 1:intel, 2:amd
  if [ $target -eq 1 ]; then
    compiler_options=" -march=icelake-server -mtune=icelake-server"
    target="icelake"
